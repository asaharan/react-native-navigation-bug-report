...
} = require("react-native-keyboard-tracking-view");
...
const renderInput = () =>
  showTextInputToTestKeyboardInteraction && (
    <TextInput
      placeholder="Input"
      style={{ borderWidth: 1, borderRadius: 10, height: 40, width: 100 }}
    />
  );
const renderFooterText = props =>
  props.footer && <Text style={styles.footerText}>{props.footer}</Text>;
const renderComponentId = props => (
  <Text style={styles.footerText}>{`this.props.componentId = ${
    props.componentId
  }`}</Text>
);
